...
type DataMode = "none" | "science" | "battle";
...
const DATA_MODE_HELP: Record<DataMode, string> = {
  none: "Just the AI-generated scenes — nothing extra on screen.",
  science: "For factual subjects (a planet, a discovery, a named scientist) it pulls a REAL image from Wikipedia instead of an AI one, with the name on screen.",
  battle: "Adds on-screen stat cards (e.g. weight / bite force / speed) for the matchup.",
};
  ...
  return (
    <div>
      ...
      <p style={{ color: "var(--fg-muted)", marginBottom: 16, lineHeight: 1.6 }}>
        Save a separate prompt set per channel so different content (science, battles, …) never gets
        mixed up. Pick a channel on the <strong>New run</strong> page and the run uses its prompts and
        data mode. A new channel starts from your global <a href="/prompts">Prompts</a> defaults — edit
        from there.
      </p>
      ...
      <div style={{ display: "flex", gap: 20, alignItems: "flex-start", flexWrap: "wrap" }}>
            ...
            {channels.map((c) => {
              ...
              return (
                ...
                >
                  ...
                  <span className={`badge ${c.data_mode === "none" ? "badge-neutral" : "badge-accent"}`}>
                    {c.data_mode === "none" ? "plain" : c.data_mode}
                  </span>
                </button>
              );
            })}
        ...
        {/* Editor */}
        {editing ? (
          <div className="card" style={{ flex: 1, minWidth: 360, display: "grid", gap: 12 }}>
            <div>
            ...
            <div>
              <label className="label">Content / data mode</label>
              <select
                className="input"
                value={editing.data_mode}
                onChange={(e) => set("data_mode", e.target.value as DataMode)}
              >
                <option value="none">Plain — AI scenes only</option>
                <option value="science">Science — real images from Wikipedia</option>
                <option value="battle">Battle — on-screen stat cards</option>
              </select>
              <p style={{ color: "var(--fg-faint)", fontSize: 12.5, marginTop: 6, lineHeight: 1.5 }}>
                {DATA_MODE_HELP[editing.data_mode]}
              </p>
            </div>
