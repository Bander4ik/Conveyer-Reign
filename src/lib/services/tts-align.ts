 ...
 *   makes audible breaks every 4-6 seconds (a single full-script call sounds
 *   fluid, the exact same script split into per-scene calls sounds choppy).
 *   The fix isn't a setting tweak — it's an architectural change.
 *
 * Strategy:
 ...
 * Whisper transcription + greedy alignment. Caller passes the audio duration.
 */
 ...
 *   multipart/form-data: file, model=whisper-large-v3,
...
async function transcribeWithGroqWhisper(audioPath: string): Promise<TranscriptWord[]> {
  ...
  if (!apiKey) {
    throw new Error(
      "GROQ_API_KEY is not set — required for single-shot voiceover mode. Paste it in Settings (get one free at console.groq.com), or set TTS_MODE back to per-scene."
    );
  }
 ...
 * "five") or skips/inserts a word. The matcher tolerates small drift before
 * nudging the cursor to resync. Scenes whose words couldn't be located are
 * filled in by interpolating from neighbors. After that, boundaries are
 * smoothed: scene N+1 starts exactly where scene N ended; the last scene runs
 * to the total audio duration. This guarantees a hole-free timeline for the
 * assembler regardless of how rough alignment was.
 */
function alignScenesToTranscript(
  ...
  // Fill un-aligned scenes from neighbors' anchors.
  ...
  // Hard cap per scene at 30s. If a single scene exceeds 30s (Whisper failed to
  // align mid-script), cap and shift the excess into the next scene — audio
  // playing over a slightly-wrong visual beats a multi-minute freeze frame.
  ...
  // The LAST scene must ALWAYS reach the true audio end (never truncate — that
  // would drop the tail of the narration in the final mux).
  if (raw.length > 0) {
